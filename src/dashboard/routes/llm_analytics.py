        ...
        try:
            ...
            summary_row = conn.execute("""
                SELECT
                    ...
                    ROUND(AVG(prompt_tokens + completion_tokens)::numeric, 0) AS avg_total_tokens,
                ...
                WHERE ts >= %s
            """ + exch_frag, (cutoff, *exch_params)).fetchone()
            summary = dict(summary_row) if summary_row else {}

            # -- Time series ----------------------------------------------------
            ts_rows = conn.execute(f"""
                SELECT
                    to_char(ts::timestamp, '{bucket_fmt}') AS bucket,
                    COUNT(*) AS calls,
                    ...
                    ROUND(AVG(latency_ms)::numeric, 0) AS avg_latency_ms
                FROM agent_reasoning
                WHERE ts >= %s""" + exch_frag + """
                GROUP BY bucket
                ORDER BY bucket ASC
            """, (cutoff, *exch_params)).fetchall()
            ...
            latency_rows = conn.execute("""
                SELECT latency_ms
                FROM agent_reasoning
                WHERE ts >= %s AND latency_ms IS NOT NULL
            """ + exch_frag + """
    ...
    with db._get_conn() as conn:
        try:
            ...
            signal_rows = conn.execute("""
                ...
                FROM agent_reasoning
                WHERE ts >= %s AND agent_name = 'strategist' AND signal_type IS NOT NULL
            """ + exch_frag + """
            ...
            total_strat = conn.execute(
                "SELECT COUNT(*) AS cnt FROM agent_reasoning WHERE ts >= %s AND agent_name = 'strategist'"
                + exch_frag,
                (cutoff, *exch_params)
            ).fetchone()
            context["total_strategist_calls"] = total_strat["cnt"] if total_strat else 0
