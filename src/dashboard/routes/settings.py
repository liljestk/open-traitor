...
def get_setup_config():
    ...
    try:
        ...
        state = {
            ...
            "redditUserAgent": env("REDDIT_USER_AGENT", "opentraitor/1.0"),
...
def setup_config(body: _SetupConfigBody, request: Request):
    ...
    try:
        ...
        # Preserve existing keys not managed by the wizard (e.g. DATABASE_URL,
        # TRAITOR_DB_PASSWORD, DASHBOARD_PASSWORD_HASH).  Without this, a
        # full-file rewrite from only the wizard payload silently drops them.
        preserved_keys: dict[str, str] = {}
        if existing_env:
            for k, v in existing_env.items():
                if k not in filtered_config_env:
                    preserved_keys[k] = v

        # Build file content with comments
        config_lines = [
            "# ===========================================",
            "# OpenTraitor Environment Configuration",
        ...
        if preserved_keys:
            config_lines.append("")
            config_lines.append("# Preserved keys (not managed by setup wizard)")
        ...
        # Preserve existing root env keys not covered by the wizard payload
        existing_root_env = _parse_env_file(root_env_path)
        if not existing_root_env:
            existing_root_env = _parse_env_file(".env")
        preserved_root: dict[str, str] = {}
        if existing_root_env:
            for k, v in existing_root_env.items():
                if k not in filtered_root_env:
                    preserved_root[k] = v
        ...
        if preserved_root:
            root_lines.append("")
            root_lines.append("# Preserved keys (not managed by setup wizard)")
            for key, value in preserved_root.items():
                root_lines.append(f"{key}={str(value).replace(chr(10), '').replace(chr(13), '')}")
        root_lines.append("")
