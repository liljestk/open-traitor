...
def _fetch_recent_trades_with_reasoning(
    ...
    """Outcomes joined with the strategist reasoning that produced them.

    Uses ``trading_outcomes`` (pnl_pct, exit_reason) and
    ``agent_reasoning`` (matched on cycle_id + pair).
    """
    since = datetime.now(timezone.utc) - timedelta(days=int(lookback_days))
    sql = (
        "SELECT t.cycle_id, t.pair, t.pnl_pct, t.exit_reason, "
        "       r.agent_name, r.reasoning_json, r.ts AS decided_at "
        "FROM trading_outcomes t "
        "JOIN agent_reasoning r "
        "  ON r.cycle_id = t.cycle_id "
        "  AND r.pair = t.pair "
        "  AND r.exchange = t.exchange "
        "WHERE t.exchange = %s "
        "  AND r.ts >= %s "
    ...
    try:
        with db._get_conn() as conn:
            rows = conn.execute(
                sql, (exchange, since.isoformat(), int(limit))
            ).fetchall()
...
def compute_attribution(
    ...
    for r in rows_in:
        ...
        seen_trades.add((r.get("cycle_id"), r.get("pair")))
    ...
    if out_rows:
        ...
        for r, src in zip(out_rows, rows_in * (1 + len(out_rows) // max(1, len(rows_in)))):
            key = (src.get("cycle_id"), src.get("pair"))
    ...
    *,
    ...
    async def _replay_one(r):
        ...
        async with sem:
            ...
            try:
                actual_pnl = float(r.get("pnl_pct") or 0.0)
            except (TypeError, ValueError):
                actual_pnl = 0.0
