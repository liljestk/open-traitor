...
class SpanContext:
    ...
    _exchange: str = field(default="", repr=False)
...
class TraceContext:
    ...
    def __init__(
        ...
        # Capture the real Langfuse trace ID (auto-generated by the SDK)
        self._langfuse_trace_id: str = ""
        if trace is not None:
            try:
                self._langfuse_trace_id = getattr(trace, "trace_id", "") or ""
            except Exception:
                pass
    ...
    def trace_id(self) -> str:
        """The real Langfuse trace ID (for building trace URLs), falling back to cycle_id."""
        return self._langfuse_trace_id or self._cycle_id
...
class LLMTracer:
    ...
    def get_trace_url(self, trace_id: str) -> Optional[str]:
        """Return the full Langfuse UI URL for a trace, or None if unavailable."""
        if self._langfuse is not None:
            try:
