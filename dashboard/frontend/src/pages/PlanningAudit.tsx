    ...
    <div className="p-6 space-y-5">
      ...
      {activeTab === 'plans' && (
        <>
          ...
          {plansLoading && <p className="text-gray-500 text-sm">Loading plans…</p>}
          <div className="space-y-3">
            {plansData?.plans.map((plan) => <PlanRow key={plan.id} plan={plan} />)}
            {!plansLoading && (plansData?.plans.length ?? 0) === 0 && (
              <p className="text-gray-500 text-sm">No plans found. Plans are generated by Temporal workflows.</p>
            )}
          </div>
        </>
      )}
      ...
      {activeTab === 'temporal' && (
        <>
          {runsLoading && <p className="text-gray-500 text-sm">Loading Temporal runs…</p>}
          {runsIsError && (
          ...
          <div className="space-y-3">
            ...
            {!runsLoading && (runsData?.runs.length ?? 0) === 0 && !runsIsError && (
              <p className="text-gray-500 text-sm">No Temporal runs found.</p>
            )}
          </div>
        </>
      )}
